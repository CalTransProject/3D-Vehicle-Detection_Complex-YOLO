...
from data_process import practice_data_utils, practice_bev_utils
from data_process.practice_dataloader import create_test_dataloader
...
from utils.visualization_utils_practice import show_image_with_boxes, merge_rgb_to_bev, predictions_to_practice_format
...
def parse_test_configs():
    ...
    ####################################################################
    configs.working_dir = '../'
    configs.dataset_dir = os.path.join(configs.working_dir, 'dataset', 'practice')
        ...
        for batch_idx, (img_paths, imgs_bev) in enumerate(test_dataloader):
            ...
            for detections in img_detections:
                ...
                for x, y, w, l, im, re, *_, cls_pred in detections:
                    ...
                    # Draw rotated box
                    practice_bev_utils.drawRotatedBox(img_bev, x, y, w, l, yaw, cnf.colors[int(cls_pred)])

            img_rgb = cv2.imread(img_paths[0])
            calib = practice_data_utils.Calibration(img_paths[0].replace(".png", ".txt").replace("image_2", "calib"))
            # objects_pred = predictions_to_kitti_format(img_detections, calib, img_rgb.shape, configs.img_size)
            objects_pred = predictions_to_practice_format(img_detections, calib, img_rgb.shape, configs.img_size)
