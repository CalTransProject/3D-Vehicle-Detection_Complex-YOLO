...
def show_image_with_boxes(img, objects, calib, show3d=False):
    ...
    for obj in objects:
        ...
        box3d_pts_2d, box3d_pts_3d = practice_data_utils.compute_box_3d(obj, calib.P)
        if box3d_pts_2d is not None:
            img2 = practice_data_utils.draw_projected_box3d(img2, box3d_pts_2d, cnf.colors[obj.cls_id])
    if show3d:
        cv2.imshow("img", img2)
    return img2
...
def show_lidar_with_boxes(pc_velo, objects, calib,
    ...
    for obj in objects:
        ...
        # Draw heading arrow
        ori3d_pts_2d, ori3d_pts_3d = practice_data_utils.compute_orientation_3d(obj, calib.P)
...
def invert_target(targets, calib, img_shape_2d, RGB_Map=None):
    ...
    for index, l in enumerate(predictions):
        ...
        _, corners_3d = practice_data_utils.compute_box_3d(obj, calib.P)
    ...
    if RGB_Map is not None:
        ...
        practice_bev_utils.draw_box_in_bev(RGB_Map, target)

    return objects_new


def predictions_to_practice_format(img_detections, calib, img_shape_2d, img_size, RGB_Map=None):
    predictions = []
